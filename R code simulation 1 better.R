...
# Modified Linear interaction model with asymmetry
delta_linear_modified <- function(Cn, Xn) {
  ...
  b <- matrix(c(0.2, 0.1, 0.4,
                0.4, 0.2, 0.1,
                0.1, 0.4, 0.2), nrow = K, ncol = K, byrow = TRUE)
...
validate_clt_improved <- function(simulation, title) {
  ...
  # Find the latest index where variance is non-zero
  variances <- apply(Zn, 2, var)
  if (all(variances == 0)) {
    warning("Variance of fluctuations is zero throughout. Cannot perform CLT validation.")
    return(NULL)
  }
  
  # Determine the point where variance becomes zero
  zero_var_indices <- which(variances == 0)
  if (length(zero_var_indices) > 0) {
    # Use data up to the smallest index where variance becomes zero
    max_N <- max(which(apply(Zn, 1, function(row) all(row != 0))))
    Zn <- Zn[1:max_N, , drop = FALSE]
    N <- nrow(Zn)
    Tn <- Tn[1:N]
    Pn <- Pn[1:N, ]
  }
  
  # Take last n_steps steps for analysis
  n_steps <- min(1000, N)
